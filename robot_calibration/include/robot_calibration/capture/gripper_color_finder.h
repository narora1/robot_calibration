...
#ifndef ROBOT_CALIBRATION_CAPTURE_GRIPPER_COLOR_FINDER_H
#define ROBOT_CALIBRATION_CAPTURE_GRIPPER_COLOR_FINDER_H
...
class GripperColorFinder : public FeatureFinder
{
  ...
  struct CloudDifferenceTracker
  {
    ...
    bool isFound(const sensor_msgs::Image& image,
                 double threshold);

    // Gives a refined centroid using multiple points
    bool getRefinedCentroid(const sensor_msgs::Image& image,
...
public:
  GripperColorFinder(ros::NodeHandle & n);
...
#endif  // ROBOT_CALIBRATION_CAPTURE_GRIPPER_COLOR_FINDER_H
