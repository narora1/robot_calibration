...
GroundPlane(Camera3dModel* camera_model, double* z = 0,
...
bool operator()(double const * const * free_params,
                  double* residuals) const
  {
    ...
    {
      residuals[i] = camera_pts[i].point.z - z; // if camera_pts is in base frame
      //residuals[i] = camera_pts[i].point.y - z; // if camera_pts is in camera frame
    }
    return true;  
  }
