...
FreightGroundFinder::FreightGroundFinder(ros::NodeHandle & nh) :
  FeatureFinder(nh),
  waiting_(false)
{
  ...
  subscriber_ = nh.subscribe(topic_name,
                             1,
                             &FreightGroundFinder::cameraCallback,
                             this);
...
void FreightGroundFinder::cameraCallback(const sensor_msgs::PointCloud2& cloud)
{
  if (waiting_)
  {
    cloud_ = cloud;
...
bool FreightGroundFinder::waitForCloud()
{
...
bool FreightGroundFinder::find(robot_calibration_msgs::CalibrationData * msg)
{
  ...
  for (size_t i = 0; i < num_points; i++)
  {
    ...
    if (p.z > 1.8)  //  pmd doesnt see the ground at a further distance
      continue;
    (iter + j)[X] = (xyz + i)[X];
    (iter + j)[Y] = (xyz + i)[Y];
    (iter + j)[Z] = (xyz + i)[Z];
