...
bool GripperDepthFinder::find(robot_calibration_msgs::CalibrationData * msg)
{
  ...
  // convert the depth image to points to find the actual cluster
  cv::Mat points3d;
  ...
  try
  {
    ...
    { 
      ...
      //    ros::Time time;
      listener.waitForTransform(  "/head_camera_depth_optical_frame"/*image_->header.frame_id*/, point.frame_id_,
          time, ros::Duration(3.0));
